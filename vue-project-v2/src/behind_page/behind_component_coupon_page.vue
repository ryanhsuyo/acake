...
export default {
  ...
  methods: {
    ...
    updateCoupon(){
      ...
      if(this.memberID && this.discount && this.useThreshold != "-1" && this.deadline && this.quantity){
        axios.post("http://localhost/yoyo/BE_updateCoupon.php", qs.stringify({memberID: this.memberID, discount: this.discount, threshold: this.useThreshold, expiration: this.deadline, quantity: this.quantity}))
        .then(res => {
          // console.log(res)
          alert("成功送出折價券！");
          this.memberID = "";
  ...
  mounted() {
    axios.post("http://localhost/yoyo/BE_selectAllMemberID.php")
      .then(res => {
