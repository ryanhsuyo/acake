...
<script>
    ...
    export default {
        ...
        methods:{
            deleteFav(cakeID, index){
                if(confirm("確定要將此蛋糕從收藏中刪除嗎?")){
                    axios.post("http://localhost/yoyo/deleteFavorite.php",qs.stringify({favoriteCategoryID: this.favCategoryID, cakeID: parseInt(cakeID)}))
        ...
        mounted(){
            axios.post("http://localhost/yoyo/selectFavDetail.php",qs.stringify({memberID: this.memberID, favCategoryID: this.categoryID}))
                    .then(res => {
                        let data = res["data"];
